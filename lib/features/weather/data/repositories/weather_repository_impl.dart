...
class WeatherRepositoryImpl implements WeatherRepository {
  ...
  Future<Either<Failure, WeatherEntity>> getCurrentWeatherByCoordinates({
    ...
    if (await networkInfo.isConnected) {
      try {
        ...
        final remoteWeather = await remoteDatasource
        ...
        return Left(
          GenericFailure(message: 'Failed to fetch weather: ${e.toString()}'),
        );
      }
    } else {
  ...
  Future<Either<Failure, WeatherEntity>> getCachedWeather({
    ...
    try {
      ...
      return Left(
        CacheFailure(message: 'No cached weather found for this location'),
      );
    } on CacheException catch (e) {
      // Cache-specific errors
      return Left(CacheFailure(message: e.message));
